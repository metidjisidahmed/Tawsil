...
const AdSearch =({  adStatus ,openEditDialog , isTransporter,status , image , title , details , price , productType , volume , weight , created_at , wilaya , ad_id , owner})=>{
    ...
    return(
        <div className="w-100 d-flex mt-lg-4 mb-lg-4 main-black-bg with-shadow " >
            <div className="col-4 pl-0">
                <img style={{ height: "15rem" , width: "20rem"}} src={image} />
            </div>
                    ...
                    <div className="d-lg-flex justify-content-lg-between align-items-center mb-lg-2">
                        ...
                        {/*<div className="main-green pt-lg-2"> <VerifiedUser/> Guaranteed </div>*/}
                        {
                            isTransporter ?(
                                ...
                                adStatus === "ARCHIVED" ?  <div className="pt-lg-2 adBadgeContainer"><Alert variant="filled" severity="error">DELETED !</Alert> </div> :
                                    adStatus === "DEACTIVATED" ? <div className="pt-lg-2 adBadgeContainer"><Alert variant="filled" severity="error">Not validated yet !</Alert> </div>: (
                                        status==="TRANSPORTER_CONFIRMING" ?  <div  className="pt-lg-2 adBadgeContainer"><Alert variant="filled" severity="success">Delivered !</Alert> </div>
                                            ...
                                            : status==="TRANSPORTER_POSTULING"  ? <div className="pt-lg-2 adBadgeContainer"><Alert variant="filled" severity="info">Waiting your confirmation !</Alert> </div>
                                                : status==="USER_REJECTING" || status==="TRANSPORTER_REJECTING" ? <div className="pt-lg-2 adBadgeContainer"><Alert variant="filled" severity="error">Rejected !</Alert> </div> :
                                                    <div className="pt-lg-2 adBadgeContainer"><Alert variant="filled" severity="info">No delivery requests actually !</Alert> </div>
                                    )
                            )
                        }
                    </div>
                    ...
                    <div className="offset-1 col-4 d-flex flex-column " >
                        ...
                        <div style={{height : "fit-content"}} className="main-black-bg p-3 with-shadow">
                            <div className="d-flex justify-content-center align-items-center flex-column">
                                <IconButton onClick={()=>setEditProfileDialogStatus(true)} style={{position : "absolute" , top : "6.5rem" , right :"2.5rem" }} className="main-white" >
                                    <ModeEdit />
                                </IconButton>
                ...
                <div className="d-lg-flex flex-column offset-lg-2 col-lg-8">
                    { adsType === "client" ? (
                            user.data.ads.asClient.map((ad , index)=> {
                                console.log("Ad =", ad);
                                return (
                                    <AdSearch adStatus={ad.ad_status} owner openEditDialog={openEditDialog}
                                              ad_id={ad.ad_id} status={ad.delivery_request_status}
                                              price={ad.ad_final_price != null ? ad.ad_final_price + "DA" : "Negotiated"}
                                              type={ad.type} productType={ad.name}
                                              image={endpoints.BASE_URL_ADS + ad.image}
                                              ...
                                              wilaya={`${wilayasLookup[Number(ad.wilaya_from)]} -> ${wilayasLookup[Number(ad.wilaya_to)]}`}/>
                                )
                            })
                        )
                        : (
